...
jQuery(document).ready(function (){
	...
	jQuery(".bannertitle").css("left", jQuery("#header-core").css("margin-left"));
	jQuery(".customfooterinner").css("left", jQuery("#header-core").css("margin-left"));
	var x = jQuery(".featured-title span").html(); 
	var y = x.split(" ");
	if(y[2]) {
		y[2] = "<p class='titlebolder'>" + y[2] + "</p>";
	}
	jQuery(".featured-title span").html(y.join(" "));
	jQuery(".featured-link a span").html("LEARN MORE");
});
